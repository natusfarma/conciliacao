...
namespace ConciliacaoBancaria_GUI.Consulta
{
    public partial class Frm_ConsultaNaoLocalizados : Form
    {
        ...
        private void Frm_ConsultaNaoLocalizados_Load(object sender, EventArgs e)
        {
        ...
        private void dgvLanctoDebitos_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            foreach (DataGridViewRow row in dgvLanctoDebitos.Rows)
            {
                if (Convert.ToString(row.Cells[8].Value) == "Itec")
                {
